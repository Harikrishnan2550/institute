...
const qustions = [
  { id: 1, type: "button", question: "What is your experience level?", options: ["Beginner", "Intermediate", "Advanced"] },
  { id: 2, type: "button", question: "Do you have previous IT experience?", options: ["Yes", "No"], inputPlaceholder: "Enter your previous role" },
  { id: 3, type: "button", question: "What is your education level?", options: ["High School", "Bachelor's", "Master's", "Other"] },
  { id: 4, type: "input", question: "Year of passing?", inputPlaceholder: "Enter year" },
  { id: 5, type: "button", question: "What is your interest area?", options: ["Web Development", "Data Science", "Mobile Apps", "AI/ML"] },
  { id: 6, type: "button", question: "Are you aware of job opportunities?", options: ["Yes", "No", "Somewhat"] },
  { id: 7, type: "button", question: "Preferred domain?", options: ["Frontend", "Backend", "Full Stack", "DevOps"] },
  { id: 8, type: "input", question: "What is your career goal?", inputPlaceholder: "Describe your goal" },
  { id: 9, type: "button", question: "When can you start training?", options: ["Immediately", "In 1 month", "In 3 months"] },
  { id: 10, type: "button", question: "Would you like a guidance call?", options: ["Yes", "No", "Maybe later"] }
];

const IndianStates = ["Andhra Pradesh", "Karnataka", "Kerala", "Tamil Nadu", "Telangana", "Maharashtra", "Gujarat"];
...
export default function GetStarted() {
  ...
  return (
    <div className="relative bg-white rounded-xl sm:rounded-2xl lg:rounded-3xl p-4 sm:p-6 md:p-8 lg:p-12 shadow-2xl overflow-hidden">
      ...
      {/* Progress Bar */}
      <div className="flex justify-center items-center gap-1 sm:gap-2 mb-8 sm:mb-10 lg:mb-12 overflow-x-auto px-2">
        <div className="flex items-center gap-1 sm:gap-2">
          {[...Array(totalSteps)].map((_, index) => (
            <React.Fragment key={index}>
              <div
                className={`w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 rounded-full flex items-center justify-center font-semibold text-xs sm:text-sm transition-all duration-500 flex-shrink-0 ${
                  index + 1 <= currentStep
                    ? "bg-gradient-to-br from-green-300 to-green-800 text-white scale-110 shadow-lg"
                    : "bg-gray-200 text-gray-500 scale-90"
                }`}
              ...
              {index < totalSteps - 1 && (
                <div
                  className={`h-0.5 transition-all duration-500 flex-shrink-0 ${
                    index + 1 < currentStep ? "w-6 sm:w-8 lg:w-12 bg-black" : "w-3 sm:w-4 lg:w-6 bg-gray-200"
                  }`}
                ></div>
              )}
            </React.Fragment>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 w-full max-w-4xl">
                  ...
                  {currentQuestion.id === 2 && selectedOptions[2] === "Yes" && (
                    <input
                      type="text"
                      placeholder={currentQuestion.inputPlaceholder}
                      className="mt-2 sm:mt-4 w-full p-2 sm:p-3 border-2 rounded-lg text-sm sm:text-base focus:outline-none focus:border-indigo-600"
                      onChange={(e) =>
                        setSelectedOptions({
                          ...selectedOptions,
                          role: e.target.value,
