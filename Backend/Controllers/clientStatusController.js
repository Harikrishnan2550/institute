...
// 🧠 Admin or Partner — Get clients
export const getAllClientStatus = async (req, res) => {
  try {
    const { role, agentId } = req.user; // decoded from token
    let clients;

    // ✅ Admin gets all clients
    if (role === "admin") {
      clients = await CareerForm.find(
        {},
        "name q7_preferredDomain modeOfClass paymentStatus registrationStatus agentId"
      ).sort({ createdAt: -1 });
    }

    // ✅ Partner gets only their own clients
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch client status", error: error.message });
  }
};

// 🧾 Get clients by agent (Admin or Partner)
export const getClientsByAgent = async (req, res) => {
  try {
    const { role, agentId: tokenAgentId } = req.user;
    const { agentId } = req.params;
    ...
    if (role === "partner" && tokenAgentId !== agentId) {
      return res
        .status(403)
        .json({ message: "Access denied: cannot view other agent’s clients" });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch clients by agent", error: error.message });
  }
};

// ✏️ Admin — Update a client's status
export const updateClientStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const { modeOfClass, paymentStatus, registrationStatus } = req.body;
    ...
    res
      .status(500)
      .json({ message: "Failed to update client status", error: error.message });
  }
};

// console.log("✅ Partner Route Triggered for Agent ID:", agentId);

// Helper for agent fetch (used in /my-clients)
export const getClientsByAgentHelper = async (agentId) => {
  return await CareerForm.find(
    { agentId },
    "name q7_preferredDomain modeOfClass paymentStatus registrationStatus"
  ).sort({ createdAt: -1 });
};
